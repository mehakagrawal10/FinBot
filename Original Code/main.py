...
import pandas as pd
from financial_analysis import generate_market_report_perplexity, compare_to_industry_average, calculate_key_vitals, upload_to_pinecone, get_industry_averages, analyze_company_standing, generate_company_profile
import os
...
with tab2:
    ...
    if process_data:
        if not st.session_state.company_name:
            ...
            st.markdown('<p class="prompt-box">Please upload your company\'s CSV file to compare.</p>', unsafe_allow_html=True)
        else:
                ...
                if not industry_averages:
                    ...
                    try:
                        ...
                        for metric, values in comparison_results.items():
                            data.append([metric, values['Company Value'], values['Industry Average'], values['Verdict']])
                        ...
                        analysis = analyze_company_standing(company_statement, industry_averages, market_report, st.session_state.company_name)
...
with tab3:
    ...
    if st.session_state.company_name:
        ...
        st.info("ℹ️ Please enter a company name in the Overview tab to generate profile")
